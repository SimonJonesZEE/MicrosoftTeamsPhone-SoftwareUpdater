...
## 2. Installation
* Download and copy both the script module and data file to your **$HOME\Documents\WindowsPowerShell\Modules\TeamsPhoneSoftwareUpdater\1.0.1** directory if you are doing this manually.  
...
The **Status** field shows the update as `Queued` and the **SID** field contains the unique id of the update, and the **Sync** field contains a snapshot of the available update being applied.  
...
When option **6. Device restart** is initiated, the option will dynamically change to **5. Verify restart**, indicating **(3)** devices are scheduled for a reboot. 
